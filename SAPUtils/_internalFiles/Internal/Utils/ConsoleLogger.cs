...
namespace SAPUtils.Internal.Utils {
    internal static class ConsoleLogger {
        ...
        private static Logger.LogInfo _logInfo;
        
        static ConsoleLogger() {
            var config = Logger.LoadSettings();
            _logInfo = config.LogInfo;
            _minimumLevel = Enum.TryParse(config.LogLevel, true, out LogLevel level) ? level : LogLevel.Info;
        }
        private static void PrintColoredLogEntry(LogLevel level, string message,
        ...
        internal static void PrintColoredLogEntry(string timestamp, int threadId, LogLevel level, string message,
            [CallerMemberName] string callerName = "",
            [CallerFilePath] string callerFile = "",
            [CallerLineNumber] int callerLine = 0) {
            if (_minimumLevel > level) return;
        ...
        public static void Error(string message, Exception ex = null,
            ...
            [CallerFilePath] string callerFile = "",
            [CallerLineNumber] int callerLine = 0) {
            if (_minimumLevel > LogLevel.Error) return;
        ...
        public static void Critical(string message, Exception ex = null,
            ...
            [CallerFilePath] string callerFile = "",
            [CallerLineNumber] int callerLine = 0) {
            if (_minimumLevel > LogLevel.Critical) return;
